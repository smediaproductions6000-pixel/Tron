...
import { ArrowDownLeft, ArrowUpRight, ArrowLeftRight } from 'lucide-vue-next'
import { fetchBrokerTransactions } from '@/lib/api'
...
const fetchTransactions = async () => {
  loading.value = true
  try {
    const data = await fetchBrokerTransactions({ type: activeTab.value })
    filteredTransactions.value = data
  } catch (err) {
    console.error('Failed to fetch broker transactions', err)
...
TEMPLATE ALIASES
------------------------- */
const filteredTxs = computed(() => filteredTransactions.value)
...
INIT
------------------------- */
onMounted(() => { void fetchTransactions() })
</script>
    ...
    <div class="max-w-sm mx-auto px-4 pb-10">
      ...
      <div v-if="loading" class="flex justify-center items-center py-10">
        <div class="w-8 h-8 border-2 border-green-500/20 border-t-green-400 rounded-full animate-spin"></div>
           ...
           <!-- NOTICE -->
<p v-else class="text-[11px] mt-[15px] text-white/40 mb-3 leading-relaxed">
