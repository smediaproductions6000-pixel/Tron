...
import { fetchBankUsers, updateUserStatus, deleteUser } from '@/lib/api'
...
const loadUsers = async () => {
  loading.value = true
  try {
    const params: Record<string, string> = { type: 'bank' }
    if (filter.value !== 'all') params.status = filter.value

    const res = await fetchBankUsers(params)
    users.value = res.data ?? res // handle paginated or raw response
  } catch (err) {
