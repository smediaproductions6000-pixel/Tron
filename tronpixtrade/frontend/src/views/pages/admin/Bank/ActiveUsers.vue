...
import { fetchBankUsers, deleteUser } from '@/lib/api'
...
const loadUsers = async () => {
  loading.value = true
  try {
    const res = await fetchBankUsers({ type: 'bank', status: 'active' })
    users.value = (res.data ?? res) // handle both paginated or raw response
      .map(u => ({
        ...u,
    ...
    console.error('Failed to load bank active users', e)
    users.value = []
  } finally {
    loading.value = false
  }
