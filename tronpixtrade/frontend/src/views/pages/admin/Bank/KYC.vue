...
import { fetchKycUsers, approveKyc, rejectKyc, deleteKyc } from '@/lib/api'
...
const loadKyc = async () => {
  loading.value = true
  try {
    const users = await fetchKycUsers('bank')
    kycList.value = users.map((u: any) => ({
      id: u.id,
      name: u.name,
      email: u.email,
      status: 'Pending',
...
const approveSelectedKyc = async () => {
  ...
  try {
    await approveKyc(selectedUser.value.id)
...
const rejectSelectedKyc = async () => {
  ...
  actionLoading.value = true
  try {
    await rejectKyc(selectedUser.value.id)
...
const deleteSelectedKyc = async (id: number) => {
  if (!confirm('Delete this KYC entry?')) return
  try {
    await deleteKyc(id)
    kycList.value = kycList.value.filter(u => u.id !== id)
  } catch (e) {
    console.warn('Delete KYC failed, updating locally', e)
    kycList.value = kycList.value.filter(u => u.id !== id)
  }
}
</script>
