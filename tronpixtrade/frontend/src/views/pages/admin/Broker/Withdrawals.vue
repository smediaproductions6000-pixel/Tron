...
const selectedWithdrawal = ref<any | null>(null)
const searchQuery = ref('')

// Only one declaration for withdrawals
const withdrawals = ref<any[]>([])

// ------------------- COMPUTED -------------------
const filteredList = computed(() =>
  withdrawals.value.filter(w =>
    (w.user?.name || w.user?.full_name || w.user_name || w.user)
      .toString()
      .toLowerCase()
...
const approveWithdrawal = async () => {
  ...
  try {
    const res = await api.put(`/withdrawals/${selectedWithdrawal.value.id}/status`, { status: 'approved' })
    // update local status
    selectedWithdrawal.value.status = res.data.withdrawal.status ?? 'approved'
...
const rejectWithdrawal = async () => {
  ...
  actionLoading.value = true
  try {
    const res = await api.put(`/withdrawals/${selectedWithdrawal.value.id}/status`, { status: 'rejected' })
    selectedWithdrawal.value.status = res.data.withdrawal.status ?? 'rejected'
    await loadWithdrawals()
  } catch (e) {
