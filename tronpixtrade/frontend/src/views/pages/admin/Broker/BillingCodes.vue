...
const steps = ref<{ bank: Step[]; broker: Step[] }>({
  bank: [],
  broker: [],
})
...
const saveStep = async () => {
  ...
  try {
    if (editingStep.value) {
      const updated = await updateSecurityStep(editingStep.value.id, newStep.value)
      const idx = steps.value[activeType.value].findIndex(s => s.id === editingStep.value?.id)
      if (idx >= 0) steps.value[activeType.value][idx] = updated
    } else {
      const created = await createSecurityStep(newStep.value)
      steps.value[activeType.value].push(created)
    }
...
// ------------------------- LOAD DATA -------------------------
const loadSteps = async (type: 'bank' | 'broker') => {
  loading.value = true
  try {
    const data = await fetchSecuritySteps(type)
    steps.value[type] = data
  } catch (err) {
...
onMounted(() => { void loadSteps(activeType.value) })
</script>
